...
public class InventoryProvider extends ContentProvider {
    ...
    static {
        sUriMatcher.addURI(InventoryContract.CONTENT_AUTHORITY, InventoryContract.PATH_PRODUCT, PROUDCTS);

        sUriMatcher.addURI(InventoryContract.CONTENT_AUTHORITY, InventoryContract.PATH_PRODUCT + "/#", PROUDCT_ID);
    }
    ...
    private Uri insertProduct(Uri uri, ContentValues values) {
        String nameProduct = values.getAsString(InvEntry.COLUMN_PRODUCT_NAME);
        if (nameProduct == null) {
            throw new IllegalArgumentException("Product name requires");
        }

        Integer priceProduct = values.getAsInteger(InvEntry.COLUMN_PRODUCT_PRICE);
        if (priceProduct != null && priceProduct < 0) {
            throw new IllegalArgumentException("Product price requires valid");
        }

        Integer quantityProduct = values.getAsInteger(InvEntry.COLUMN_PRODUCT_QUANTITY);
        if (quantityProduct != null && quantityProduct < 0) {
            throw new IllegalArgumentException("Product quantity requires valid");
        }

        Integer supplierName = values.getAsInteger(InvEntry.COLUMN_PRODUCT_SUPPLIER_NAME);
        if (supplierName == null || !InvEntry.isValidSupplierName(supplierName)) {
            throw new IllegalArgumentException("Pet requires valid gender");
        }

        Integer supplierPhone = values.getAsInteger(InvEntry.COLUMN_PRODUCT_SUPPLIER_PHONE);
    ...
    private int updateProduct(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(InvEntry.COLUMN_PRODUCT_SUPPLIER_NAME)) {
            Integer supplierName = values.getAsInteger(InvEntry.COLUMN_PRODUCT_SUPPLIER_NAME);
            if (supplierName == null || !InvEntry.isValidSupplierName(supplierName)) {
